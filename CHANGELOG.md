...
### Changed — serialize dialyzer across worktrees to stop the harness OOM (Task 75)

- **`mix.exs` `dialyzer.json` alias** — a single `mix dialyzer.json` run on onchain's dep graph peaks at ~30 GB and that floor is irreducible (the main checkout hits ~36 GB even with a fully cached PLT; trimming `plt_add_apps` doesn't lower it and only loses ABI coverage). The 2026-06-04 host double-OOM was **four concurrent** runs (the harness reviewer's `check_command` under `concurrency_cap=4`), not a single avoidable cold build — so the original task premise (seed the PLT to skip the build) could not have fixed it. The fix shadows `dialyzer.json` with `with_dialyzer_lock/1`: a host-global, atomic **mkdir lock** under `~/.cache/onchain-dialyzer.lock` that serializes runs to one-at-a-time (peak ~30 GB instead of 4×). macOS has no `flock(1)`, hence mkdir; a lock older than 30 min (a crashed run) is reclaimed so a dead process can't wedge the host. Verified: two concurrent `mix dialyzer.json` runs queue (the second prints `Waiting for onchain dialyzer lock…` and starts only after the first releases); both complete green.
- **`seed_plts/0`** (time-saver, not the OOM fix) — when invoked from a linked git worktree, copies the main checkout's already-built **core** PLTs (`dialyxir_erlang-*` / `…_elixir-*`) into the worktree so dialyxir skips the per-worktree core build (~5s md5 verify vs a full build; core beams live in the shared asdf install at a constant path). The deps PLT can't be seeded — dep beam `debug_info` embeds the absolute worktree path, so its md5s differ per worktree and dialyxir rebuilds it regardless. No-op in the main checkout, when already seeded, or when no source is reachable. `plt_local_path`/`plt_core_path` stay `priv/plts`, so main-checkout behavior is unchanged.
