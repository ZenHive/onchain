...
- **Bundles with Task 43 (next commit).** Cartouche corrected the `Cartouche.Hex` specs that signet had wrong, but the `@dialyzer {:no_match, ...}` suppressions across every `Onchain.*` module that flows through `ABI.decode_response/2` (`Onchain.ABI`, `Onchain.Hex`, `Onchain.Contract`, `Onchain.ENS`, `Onchain.Log`, `Onchain.Multicall`, `Onchain.Sleuth`, `Onchain.ERC20`, `Onchain.ERC721`, `Onchain.ERC1155`, `Onchain.Transfer`) remain in place for this commit — Task 43 strips them in a separate commit immediately after this one, both shipping in `v0.5.2`.
